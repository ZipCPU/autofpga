...
`include "builddate.v"
	...
	wire	[7:0]	wb_skip; // bits 01100f90, sbaw=23
	assign	wb_skip = {
			wb_addr[22],
			wb_addr[18],
			wb_addr[ 9: 5],
	...
	assign	        pmic_sel = ( wb_skip[ 7: 0] ==  8'b0010_0000_);
	assign	       flctl_sel = ( wb_skip[ 7: 0] ==  8'b0010_0001_);
	assign	         cfg_sel = ( wb_skip[ 7: 1] ==  7'b0010_001);
	assign	        mdio_sel = ( wb_skip[ 7: 1] ==  7'b0010_010);
	assign	         mem_sel = ( wb_skip[ 7: 6] ==  2'b01);
	assign	       flash_sel = ( wb_skip[ 7: 7] ==  1'b1);
	assign	         dio_sel = ( wb_skip[ 7: 3] ==  5'b0001_1);
	assign	         sio_sel = ( wb_skip[ 7: 1] ==  7'b0001_000);
	assign	none_sel = (wb_stb)&&({ sio_sel, dio_sel, flash_sel, mem_sel, mdio_sel, cfg_sel, flctl_sel, pmic_sel} == 0);
	//
	// many_sel
	//
...
`ifdef	VERILATOR
	...
	always @(*)
		case({sio_sel, dio_sel, flash_sel, mem_sel, mdio_sel, cfg_sel, flctl_sel, pmic_sel})
			8'h0: many_sel = 1'b0;
...
`endif	// VERILATOR
	...
	always @(posedge i_clk)
		case({sio_ack, dio_ack, flash_ack, mem_ack, mdio_ack, cfg_ack, flctl_ack, pmic_ack})
			8'h0: many_ack <= 1'b0;
	...
	always @(posedge i_clk)
		dio_ack <= pre_dio_ack;
	always @(posedge i_clk)
		wb_ack <= (wb_cyc)&&(|{sio_ack, dio_ack, flash_ack, mem_ack, mdio_ack, cfg_ack, flctl_ack, pmic_ack});
...
`endif	// HDMI_IN_EDID_ACCESS
	...
	always @(posedge i_clk)
	begin
		casez({ sio_ack, dio_ack, flash_ack, mem_ack, mdio_ack, cfg_ack, flctl_ack, pmic_ack })
			8'b1???????: wb_idata <= sio_data;
			8'b01??????: wb_idata <= dio_data;
			8'b001?????: wb_idata <= flash_data;
			8'b0001????: wb_idata <= mem_data;
			8'b00001???: wb_idata <= mdio_data;
			8'b000001??: wb_idata <= cfg_data;
			8'b0000001?: wb_idata <= flctl_data;
			8'b00000001: wb_idata <= pmic_data;
