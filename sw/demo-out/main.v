...
`include "builddate.v"
	...
	assign	        mous_sel = (dio_sel)&&(dio_skip[ 3: 0] ==  4'b0001_);
	assign	        oled_sel = (dio_sel)&&(dio_skip[ 3: 0] ==  4'b0010_);
	assign	         rtc_sel = (dio_sel)&&(dio_skip[ 3: 0] ==  4'b0011_);
	assign	         gtb_sel = (dio_sel)&&(dio_skip[ 3: 1] ==  3'b010);
	assign	        edin_sel = (dio_sel)&&(dio_skip[ 3: 3] ==  1'b1);
	wire	[6:0]	wb_skip; // bits 01100790, sbaw=23
	assign	wb_skip = {
			wb_addr[22],
			wb_addr[18],
			wb_addr[ 8: 5],
	...
	assign	       flctl_sel = ( wb_skip[ 6: 0] ==  7'b001_1000_);
	assign	        gpsu_sel = ( wb_skip[ 6: 0] ==  7'b001_1001_);
	assign	         cfg_sel = ( wb_skip[ 6: 1] ==  6'b001_101);
	assign	        mdio_sel = ( wb_skip[ 6: 1] ==  6'b001_110);
	...
	assign	none_sel = (wb_stb)&&({ sio_sel, dio_sel, flash_sel, mem_sel, mdio_sel, cfg_sel, gpsu_sel, flctl_sel} == 0);
	//
	// many_sel
	//
...
`ifdef	VERILATOR
	...
	always @(*)
		case({sio_sel, dio_sel, flash_sel, mem_sel, mdio_sel, cfg_sel, gpsu_sel, flctl_sel})
			8'h0: many_sel = 1'b0;
...
`endif	// VERILATOR
	...
	always @(posedge i_clk)
		case({sio_ack, dio_ack, flash_ack, mem_ack, mdio_ack, cfg_ack, gpsu_ack, flctl_ack})
			8'h0: many_ack <= 1'b0;
	...
	always @(posedge i_clk)
		dio_ack <= pre_dio_ack;
	always @(posedge i_clk)
		wb_ack <= (wb_cyc)&&(|{sio_ack, dio_ack, flash_ack, mem_ack, mdio_ack, cfg_ack, gpsu_ack, flctl_ack});
...
`ifdef	HDMI_IN_EDID_ACCESS
	wbi2cslave	the_input_edid(i_clk, 1'b0,
...
`endif	// MOUSE_ACCESS
	...
	always @(posedge i_clk)
	casez({ gck_ack, mous_ack, oled_ack, rtc_ack, gtb_ack, edin_ack })
		 6'b1?????: dio_data <= gck_data;
		 6'b01????: dio_data <= mous_data;
		 6'b001???: dio_data <= oled_data;
		 6'b0001??: dio_data <= rtc_data;
		 6'b00001?: dio_data <= gtb_data;
		 6'b000001: dio_data <= edin_data;
		default: dio_data <= 32'h0;
	endcase
	always @(posedge i_clk)
	begin
		casez({ sio_ack, dio_ack, flash_ack, mem_ack, mdio_ack, cfg_ack, gpsu_ack, flctl_ack })
			...
			8'b0000001?: wb_idata <= gpsu_data;
			8'b00000001: wb_idata <= flctl_data;
